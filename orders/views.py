...
from .sms import make_post_request
...
class CustomerViewAll(generics.GenericAPIView):
    """Handle individual customer detail actions"""
    serializer_class = UserCreateSerializer
...
class CustomerByIdView(generics.GenericAPIView):
    """Handle customer detail retriev by its ID"""
...
class ProductView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(operation_summary="Create an item") # document the API 
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            serializer.save()
...
class ProductDetailView(generics.GenericAPIView):
    """Handle individual item detail actions"""
    queryset = Item.objects.all()
    serializer_class = ProductCreateSerializer
    permission_classes = [IsAuthenticated]
...
class OrderViewById(generics.GenericAPIView):
    """Handle order retrieval by its ID"""
    serializer_class = OrderCreateSerializer
    queryset = Order.objects.all()
...
class OrderDetailById(generics.GenericAPIView):
    """Handle order detail retri by its ID"""
    queryset = OrderDetail.objects.all()
    serializer_class = OrderDetailsSerializer
    permission_classes = [IsAuthenticated]
...
class OrderSearchView(generics.GenericAPIView):
    """admin to search for orders by date range"""
    serializer_class = OrderDetailsSerializer
    ...
    # caches the search results based on the date range
    def get(self, request):
